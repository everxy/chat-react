...
var bodyParser = require('body-parser'); //parsering POST request data
...
app.put('/api/characters', function(req, res, next) {
  ...
  //async.parallel(tasks,callback)方法，并行执行所有的函数任务，当成功返回以数组形式给callback函数，任意任务失败返回callback中的err处理
  async.parallel([
      function(callback) {
        Character.findOne({
          characterId: winner
